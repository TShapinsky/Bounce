...
def generate_model(pts):
    ...
    if len(xs) > 1:
        return [np.polyfit(ts,xs,2), np.polyfit(ts,ys,2), np.polyfit(ts,zs,2)]
    else:
        return [float('nan')]
...
def evaluate_models(models, t):
    res = []
    for model in models:
        res.append(evaluate_model(model,t))
    return np.array(res)
...
def find_velocity(model, t):
    xdt = find_derivative(model[0],t)
    ydt = find_derivative(model[1],t)
    zdt = find_derivative(model[2],t)
...
def predict_target(points):
    if len(points) > 5:
        ...
        for i in range(len(points)-5):
            ...
            time_root = np.max(find_roots(models[2],0))
...
def to_3d_point(p1, p2):
    ...
    p1[0] += math.pi/6
    p2[0] += math.pi/6
    ray1 = Ray([20.5,0,3.75],[-1,math.tan(p1[1]),math.tan(p1[0])])
    ray2 = Ray([0,20.5,3.75],[math.tan(p2[1]),-1,math.tan(p2[0])])
